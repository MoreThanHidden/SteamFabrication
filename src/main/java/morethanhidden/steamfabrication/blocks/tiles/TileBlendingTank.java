...
public class TileBlendingTank extends TileFluidHandler implements ITickable, ISFFluidTile {
    ...
    public void updateSteam() {
        if (tank.getFluid() == null) {
            IFluidHandler lefttank = FluidUtil.getFluidHandler(this.world, pos.add(0, 0, -1), EnumFacing.WEST);
            IFluidHandler righttank = FluidUtil.getFluidHandler(this.world, pos.add(0, 0, 1), EnumFacing.EAST);
            ...
            if (lefttank != null && righttank != null) {
                ...
                if (leftfluid != null && rightfluid != null) {
                    if (leftfluid.containsFluid(new FluidStack(FluidRegistry.getFluid("water"), 1000)) && rightfluid.containsFluid(new FluidStack(FluidRegistry.getFluid("lava"), 1000))) {
                        active = true;
                        count++;
                        world.notifyBlockUpdate(pos, world.getBlockState(pos), world.getBlockState(pos).withProperty(BlockBlendingTank.ACTIVE, true), 0);
                        ...
                        if (count > 45) {
                            ...
                            active = false;
                            world.notifyBlockUpdate(pos, world.getBlockState(pos), world.getBlockState(pos).withProperty(BlockBlendingTank.ACTIVE, false), 0);
                        }
                        return;
                    }
                }
